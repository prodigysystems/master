...
Public Class AccountController
    ...
    <AllowAnonymous()> _
    Public Function Login(ByVal timeout As String) As ActionResult
        If Not String.IsNullOrEmpty(timeout) Then
            ModelState.AddModelError("", "Your session expired due to inactivity.  Please login again.")
        End If
        Return View()
    End Function
    ...
    Public Function Login(ByVal model As LoginModel, ByVal returnUrl As String) As ActionResult
        If ModelState.IsValid AndAlso WebSecurity.Login(model.UserName, model.Password, persistCookie:=model.RememberMe) Then
            Return RedirectToLocal(RouteConfig.GetActionUri("RemoteAuthentication", "Index"))
        End If
    ...
    Public Function Register() As ActionResult
        Dim allRoles As List(Of SelectListItem) = New List(Of SelectListItem)
        Dim availableRoles As String() = Roles.GetAllRoles()
        For Each r As String In availableRoles
            allRoles.Add(New SelectListItem With {.Text = r, .Value = r})
        Next
        ...
        If ModelState.IsValid Then
            ...
            Try
                ...
                Else
                    ...
                    ViewData("msg") = "User has been registered."
                    Return RedirectToAction("UserAdmin", "Account")
                End If
    ...
    Public Function Manage(ByVal model As LocalPasswordModel) As ActionResult
        ...
        Return View(model)
    End Function
    Public Function UserAdmin() As ActionResult
        Dim model As New UserAdminModel()
        Dim allRoles As List(Of SelectListItem) = GetAllRoles()
        Dim allUsers As List(Of RegisterModel) = GetAllUsers()
        model.Users = allUsers
        ViewData("roles") = allRoles
        Return View(model)
    End Function
    <GridAction()>
    Public Function SearchUsers() As ActionResult

        Dim allRoles As List(Of SelectListItem) = GetAllRoles()
        Dim allUsers As List(Of RegisterModel) = GetAllUsers()

        ViewData("roles") = allRoles
        Return View(New GridModel(allUsers))
    End Function
    <GridAction()>
    Public Function DeleteUser() As ActionResult
        Dim curUser As New RegisterModel
        TryUpdateModel(curUser)
        Dim curRole As String = Roles.GetRolesForUser(curUser.UserName)(0)
        Roles.RemoveUserFromRole(curUser.UserName, curRole)
        Membership.DeleteUser(curUser.UserName)
        Dim allRoles As List(Of SelectListItem) = GetAllRoles()
        Dim allUsers As List(Of RegisterModel) = GetAllUsers()
    ...
    <GridAction()>
    Public Function UpdateUser() As ActionResult
        Dim curUser As New RegisterModel
        TryUpdateModel(curUser)
        Dim oldRole As String = Roles.GetRolesForUser(curUser.UserName)(0)
        If oldRole <> "" Then
            Roles.RemoveUserFromRole(curUser.UserName, oldRole)
            Roles.AddUserToRole(curUser.UserName, curUser.Role)
        End If
        Dim allRoles As List(Of SelectListItem) = GetAllRoles()
        Dim allUsers As List(Of RegisterModel) = GetAllUsers()
    ...
    Private Function GetAllUsers() As List(Of RegisterModel)
        Dim allRoles As List(Of SelectListItem) = New List(Of SelectListItem)
        Dim allUsers As New List(Of RegisterModel)
        Dim availableRoles As String() = Roles.GetAllRoles()
        For Each r As String In availableRoles
            allRoles.Add(New SelectListItem With {.Text = r, .Value = r})
            Dim users As String() = Roles.GetUsersInRole(r)
            For Each user As String In users
                allUsers.Add(New RegisterModel With {.UserName = user, .Role = r})

            Next
        Next
        Return allUsers
    End Function
    Private Function GetAllRoles() As List(Of SelectListItem)
        Dim allRoles As List(Of SelectListItem) = New List(Of SelectListItem)
        Dim availableRoles As String() = Roles.GetAllRoles()
        For Each r As String In availableRoles
            allRoles.Add(New SelectListItem With {.Text = r, .Value = r})
